# Financial Risk Intelligence — Backend

FastAPI risk engine, PyTorch Hetero GAT, AMLSim pipeline, training scripts, and MLOps drift monitoring.

**Date**: June 1, 2026  
**Status**: Production-ready (containerized CPU serving)
...
- **Python**: 3.11+ recommended for local backend tooling; runtime serving uses Docker
- **Network Connectivity**: Ports 8000 (API) and 5173 (UI) available

### Local Environments

- Create the backend Python environment in `backend/.venv`
- If you want a matching local utility environment for the UI side, create it in `frontend/.venv`
- Avoid using a repo-root `.venv`; the codebase is now split by app boundary
