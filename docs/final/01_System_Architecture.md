...
`docker-compose.yml` defines one network-facing service:
...
`Dockerfile` builds the service on top of `python:3.11-slim` and sets:
...
The serving engine loads the archive configured in `configs/default.yaml`:

- `data/external/AMLSim/sample/20K_fanin200cycle200.tgz`

`src/fri/graph/io.py` extracts:
...
`src/fri/graph/service.py` builds a unified feature bundle directly from the archive tables.
...
`src/fri/models/pytorch_gnn.py` converts the feature bundle into a `HeteroData` graph with two node types:
...
The serving model is `SpatialTemporalHeteroGAT`, implemented in `src/fri/models/pytorch_gnn.py`.

The engine reconstructs that model from:

- the heterogenous graph shape
- the graph metrics artifact at `artifacts/graph/pytorch_gcn_metrics.json`
- the trained checkpoint file under `artifacts/graph/`

The model is loaded once during application startup and then held in memory for all subsequent requests.

### 5. API Layer

`src/fri/api/main.py` exposes the serving contract:
...
- persistent JSONL drift event logging under `artifacts/temporal/drift_events.jsonl`
...
5. The event is persisted to `artifacts/temporal/drift_events.jsonl`.
